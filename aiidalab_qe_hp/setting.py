...
class Setting(Panel):
    ...
    def get_panel_value(self):
        ...
        hubbard_u = [
            ...
            for data in self.hubbard_u_map.values() if data[0].value
        ]
        hubbard_v = [
            [data[1].value, data[2].value, data[3].value, data[4].value, data[5].value]
            for data in self.hubbard_v_map.values() if data[0].value
        ]
        parameters = {
    ...
    def set_panel_value(self, input_dict):
        ...
        for data in hubbard_v:
            checkbox, _, manifold1, _, manifold2, V = self.hubbard_v_map[(data[0], data[2])]
            checkbox.value = True
    ...
    def _generate_u(self):
        ...
        for kind_symbol in kind_list:
            checkbox = ipw.Checkbox(value=False, description='', indent=False, layout=ipw.Layout(width='20px'))
    ...
    def _generate_v(self):
        ...
        if self.input_structure is None or self.calculation_type.value != "DFT+U+V":
            print("input_structure is: ", self.input_structure)
            print("calculation_type is: ", self.calculation_type.value)
            print("skip generate_v")
            return
        structure = self.input_structure
        kind_list = [Kind.symbol for Kind in structure.kinds]
        rows = []
        for i in range(len(kind_list)):
            for j in range(i + 1, len(kind_list)):
                ...
                rows.append(ipw.HBox([checkbox, symbol1, manifold1, symbol2, manifold2, v]))
                self.hubbard_v_map[(kind_symbol1, kind_symbol2)] = [checkbox, symbol1, manifold1, symbol2, manifold2, v]
        print("hubbard_v_map: ", self.hubbard_v_map)
